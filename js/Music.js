...
Music.split = function music_splitIntoMeasures() {
  ...
  for (i = 0; i < Music.music.length; i++) {
    ...
    if (Music.music[i].fx.triplets) {
      if (
        (i < Music.music.length - 2) && (Music.music[i + 1].length == Music.music[i + 2].length) && (Music.music[i + 2].length == Music.music[i].length) && ((Music.lenSection - curLen >= Music.music[i].length))
      ) {
        curLen -= Music.music[i].length;
      } else {
  ...
  sectionsHTML = Music.sections.map(function (item, id) {
      ...
      for (i = 0; i < item.length; i++) {
        ...
        if (item[i].note.fx.triplets) {
          if (
            (i < item[i].note.length - 2) && (item[i + 1].note.length == item[i + 2].note.length) && (item[i + 2].note.length == item[i].note.length) && ((Music.lenSection - curLen >= item[i].note.length))
          ) {
            curLen -= item[i].note.length;
          }
        }
      ...
      //添加类名，并且拼接HTML
      startHTML = startHTML.replace("%CLASS%",
        className.join(" "));
...
UI.redraw = function ui_redraw() {
  ...
  Array.prototype.slice.call(document.querySelectorAll(".selected")).forEach(function (dom) {
    ...
    // 注： 这些都是指选择的元素序号。其中 from = author 为空选
    // from <- [0, length]    author <- [0,length]
  if (UI.from <= 0) UI.from = 0;
  if (UI.from >= Music.music.length) UI.from = Music.music.length;
  if (UI.author <= 0) UI.author = 0;
  if (UI.author >= Music.music.length) UI.author = Music.music.length;
  //显示光标
  ...
  if (Music.music.length == 0) {
    ...
    if (UI.editingLynicLine == -1) {
      ...
      UI.IMETip.style.top = UI.editbox.style.top = caret.style.top = (
        UI.domsAreas[Math.min(UI.author, Music.music.length - 1)].y + UI.yinheight +
...
UI.setEditor = function ui_setEditor() {
  ...
  document.addEventListener("mousedown", function ui_capture_mouse(event) {
    if (event.button == 0) UI.isSelectMouseDown = true;
  });
  document.addEventListener("mouseup",
    function ui_release_mouse() {
      UI.isSelectMouseDown = false;
    });
  document.addEventListener("mousedown",
    function ui_mousedown_to_(event) {
      if (Music.music.length == 0) {
        return;
      }
      ...
      UI.lastClickedNotePos = [event.clientX,
        event.clientY
      ];
      if (UI.isShiftDown) {
  ...
  document.addEventListener("click",
    function ui_click_focus_textbox(event) {
      if (!UI.container.contains(event.target) && event.target != document.documentElement && event.target != document.body) {
        return;
      }
      UI.editbox.focus();
  ...
  UI.container.addEventListener("contextmenu",
    UI.onContextMenu);
  document.addEventListener("click",
    function ui_click_to_hide_menu() {
      setTimeout(function () {
  ...
  UI.editbox.addEventListener("keydown",
    UI.onKeyDown);
  document.addEventListener("keydown",
    UI.onGlobalKeyDown);
  Util.onCJKInput(UI.editbox,
    UI.onInput);
  UI.editbox.addEventListener("focus",
    function show_caret() {
